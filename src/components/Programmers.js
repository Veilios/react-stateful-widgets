...
export default function Programmers() {
  ...
  let [pros] = useState(listOfAwesome);
  let [featPro, setFeatPro] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return pros[featPro - 1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (featPro === null) ? "royalblue" : "gold"
  };
      ...
      <div className='programmers'>
        {
          ...
          pros.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => {setFeatPro(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
